...
class EstimatorCV():
    ...
    def update_CV(self, features, labels):
        ...
        additional_CV = weight_CV.mul(1 - weight_CV).mul(
            torch.bmm((self.Ave - ave_CxA).view(C, A, 1), (self.Ave - ave_CxA).view(C, 1, A)))
...
class ISDALoss(nn.Module):
    ...
    def isda_aug(self, fc, features, y, labels, cv_matrix, ratio):
        ...
        NxW_ij = weight_m.to(device).expand(N, C, A)
        NxW_kj = torch.gather(NxW_ij.to(device), 1, labels.view(N, 1, 1).expand(N, C, A).to(device))
    ...
    def forward(self, model, fc, x, target_x, ratio):
        ...
        loss = LiftedLoss(isda_aug_y, target_x)
